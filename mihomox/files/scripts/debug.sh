...
`
if [ -x "/bin/opkg" ]; then
	opkg list-installed "mihomox"
	opkg list-installed "luci-app-mihomox"
elif [ -x "/usr/bin/apk" ]; then
	apk list -I "mihomox"
	apk list -I "luci-app-mihomox"
fi
`
\`\`\`
## config
...
const config = uci.get_all("mihomox");
const result = {};
...
TPROXY:
`
ip route list table "$(uci get mihomox.routing.tproxy_route_table)"
`

TUN:
`
ip route list table "$(uci get mihomox.routing.tun_route_table)"
`
\`\`\`
## ip6 rule
\`\`\`
...
TPROXY:
`
ip -6 route list table "$(uci get mihomox.routing.tproxy_route_table)"
`

TUN:
`
ip -6 route list table "$(uci get mihomox.routing.tun_route_table)"
`
\`\`\`
## nftables
\`\`\`
`
nft list table inet mihomox
`
\`\`\`
## service
\`\`\`json
`
/etc/init.d/mihomox info
`
\`\`\`
"
