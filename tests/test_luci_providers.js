...
assert.ok(viewSource.includes("_('Run Log')"));
assert.ok(viewSource.includes('mihomox-provider-log'));
assert.ok(viewSource.includes('height:12rem'));
assert.ok(viewSource.includes('updateRunLog(runLog, false)'));
assert.ok(viewSource.includes('refreshRenderedStatuses()'));
assert.ok(viewSource.includes('poll.add(() => refreshStatuses(false), 2)'));
assert.ok(viewSource.includes('mihomox.providerDiscardStatus(runLogOffset)'));
assert.ok(toolSource.includes("params: ['log_offset']"));
...
assert.ok(rpcSource.includes("const PROVIDER_FILTER_LOG = PROVIDER_FILTER_DIR + '/activity.log'"));
assert.ok(rpcSource.includes('function provider_filter_log(offset)'));
assert.ok(rpcSource.includes('if (size - start > 65536)'));
assert.ok(rpcSource.includes('log: chunk'));
assert.ok(rpcSource.includes('log_offset: log.log_offset'));
...
assert.ok(managerSource.includes('MIHOMOX_PROVIDER_FILTER_LOG_FILE'));
assert.ok(managerSource.includes('log_event "[$name] testing: $tested/$total'));
assert.ok(managerSource.includes('log_event "[$name] completed: available=$available'));
